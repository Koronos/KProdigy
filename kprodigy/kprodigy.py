...
class KProdigy(torch.optim.Optimizer):
    ...
    def _step_single_tensor(self, closure, loss):
        ...
        # Check if we should update D this step (sparse updates)
        ...
        for group in self.param_groups:
            ...
            for p in group['params']:
                ...
                if group_lr > 0.0:
                    ...
                    # D estimation (sparse updates for speed)
                    if should_update_d:
        ...
        # Compute new D estimate (only if should_update_d)
        d_hat = d
    ...
    def _step_foreach(self, closure, loss):
        ...
        # Update D only if should_update_d
        if should_update_d:
            if d_denom == 0 and not fsdp_in_use:
                return loss
    ...
    def _step_independent_d(self, closure, loss):
        ...
        for group in self.param_groups:
            ...
            # Batch operation for D denominator (only if should_update_d)
            if should_update_d:
                if s_tensors:
            ...
            # Update D only if should_update_d
            if should_update_d and d_denom > 0 and group_lr > 0.0:
                global_d_numerator = d_numerator + delta_numerator
                global_d_denom = d_denom
                ...
                # If not updating D, keep current values
                group['d'] = d
